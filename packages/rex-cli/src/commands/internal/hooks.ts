...
.option("-i --integrate", "Add support for the specified hook in current project")
...
async function hookCommand(options: any, arg: string) {
  ...
  const externAction = existsSync(`.rex/${arg}`) ? Deno.readTextFileSync(`.rex/${arg}`) : undefined;
...
async function integrateHook(arg: string, preamble: string = "", cmdName: string = "rex" ) {
  await Deno.create(`.git/hooks/${arg}`).then(f => f.writeSync(new TextEncoder().encode(`${preamble} ${cmdName} hooks ${arg} | bash`)));
}
